...
class Level extends Phaser.Scene {
	...
	editorCreate() {
		...
		// you
		const you = this.add.image(304, 711, "You");

		// opponent
		const opponent = this.add.image(1614, 711, "Opponent");
		...
		if (this.count == 0) {
			...
			this.tweens.add({
				...
				onComplete: () => {
					...
					if (this[`array${arrayIndex + 1}Y`] < 7 && enableBot == false) {
	...
	update() {
		...
		if(enableBot == true){
			if (this.count == 0) {
				...
				this.hoverCoin.setVisible(true)
			}
		}
